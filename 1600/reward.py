...
def closest(waypoints ,x1,y1):
    distances=[]
    for i in range(len(waypoints)):    
        temp=dist(waypoints[i],[x1,y1])
...
def reward_function(params):
    ...
    waypoints = [[-6.85582051,  0.87480923],
    ...
    closest_waypoints = closest(waypoints,params['x'],params['y'])
    ...
    next_point = waypoints[closest_waypoints]
    ...
    if( track_direction <=40.0):
        if params['speed']>=1.0:
            reward*=(3**params['speed'])
        else :
            reward*=0.7
